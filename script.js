...
function setupAuthUI(user) {
    ...
    const openLoginModal = (e) => { e.preventDefault(); document.getElementById('login-modal').style.display = 'block'; };
...
async function initHomepage() {
    async function fetchHomepageData() {
        ...
        const [productsSnapshot, heroDoc] = await Promise.all([getDocs(productsQuery), getDoc(heroDocRef)]);
        
        const products = productsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        const heroData = heroDoc.exists() ? heroDoc.data() : null;
        
        return { products, heroData };
    }

    function renderHero(heroData) {
        const heroSection = document.getElementById('hero-section');
        if (!heroData || !heroSection) return;
        heroSection.querySelector('.hero-subtitle').textContent = heroData.heroSubtitle || 'Our finest selection';
        heroSection.querySelector('.hero-title').textContent = heroData.heroTitle || 'Exclusive Wines';
        heroSection.querySelector('.hero-description').textContent = heroData.heroDescription || 'From around the world, delivered to you.';
        if (heroData.heroBgImage) {
    ...
    function renderFeaturedProducts(products) {
        ...
        if (featured.length > 0) {
            featured.forEach(prod => { grid.innerHTML += createProductCardHTML(prod); });
        } else {
            products.slice(0, 4).forEach(prod => { grid.innerHTML += createProductCardHTML(prod); });
        }
    }
    ...
    const { products, heroData } = await fetchHomepageData();
    renderHero(heroData);
...
async function initCatalogPage() {
    // This is the full JS logic from your original catalog.html
    let allProducts = [];
    ...
    function renderBreadcrumbs() {
        ...
        if (state.filters.country.length > 0) {
            const value = state.filters.country.sort().join(', ');
            tempFilters.country = state.filters.country.join(',');
            html += ` / <a href="/catalog.html?${new URLSearchParams(tempFilters)}">${value}</a>`;
        }
        if (state.filters.region.length > 0) {
            const value = state.filters.region.sort().join(', ');
            tempFilters.region = state.filters.region.join(',');
            html += ` / <a href="/catalog.html?${new URLSearchParams(tempFilters)}">${value}</a>`;
        }
        if (state.filters.appellation.length > 0) {
            const value = state.filters.appellation.sort().join(', ');
            tempFilters.appellation = state.filters.appellation.join(',');
            html += ` / <a href="/catalog.html?${new URLSearchParams(tempFilters)}">${value}</a>`;
        }
...
async function initProductPage() {
    // This is the full JS logic from your original product.html
    ...
    function renderProduct(product) {
        ...
        if (product.category) breadcrumbHTML += ` / <a href="/catalog.html?category=${encodeURIComponent(product.category)}">${product.category}</a>`;
        ...
        if (product.ratingValue && product.reviewCount) {
            document.getElementById('product-rating').innerHTML = `<div class="stars">${generateStars(product.ratingValue)}</div><span class="rating-value">${product.ratingValue}</span><span class="rating-count">(${product.reviewCount} reviews)</span>`;
        }
        
        const detailsContainer = document.getElementById('product-details-list');
        detailsContainer.innerHTML = '';
        const detailsOrder = [
            { key: 'country', label: 'Country', icon: 'fa-globe-americas' }, { key: 'region', label: 'Region', icon: 'fa-map-marker-alt' },
            { key: 'appellation', label: 'Appellation', icon: 'fa-award' }, { key: 'brand', label: 'Producer', icon: 'fa-industry' },
            { key: 'year', label: 'Year', icon: 'fa-calendar-alt' }, { key: 'grapeVarieties', label: 'Grapes', icon: 'fa-wine-glass' },
            { key: 'sweetness', label: 'Sweetness', icon: 'fa-tint' }, { key: 'alcohol', label: 'Alcohol', icon: 'fa-percentage', suffix: '%' },
            { key: 'volume', label: 'Volume', icon: 'fa-wine-bottle' }
        ];
        detailsOrder.forEach(detail => {
            if (product[detail.key]) { detailsContainer.innerHTML += `<div class="detail-item"><i class="fas ${detail.icon}"></i><span>${detail.label}: ${product[detail.key]}${detail.suffix || ''}</span></div>`; }
        });
        ...
        if (product.stockStatus === 'Out of Stock') {
            ...
            cartBtn.querySelector('span').innerHTML = 'Out of Stock';
        } else {
            stockEl.classList.add(product.stockStatus === 'Low Stock' ? 'low-stock' : 'in-stock');
            cartBtn.disabled = false;
        }

        const tastingNotesTab = document.getElementById('tasting-notes-tab');
        if (product.tastingNotes) {
            const notesHTML = product.tastingNotes.split('\n').map(line => { const [key, ...value] = line.split(':'); return `<li><strong>${key}:</strong> ${value.join(':').trim()}</li>`; }).join('');
            tastingNotesTab.innerHTML = `<h3>Tasting Notes</h3><ul class="tasting-notes-list">${notesHTML}</ul>`;
        } else {
            tastingNotesTab.innerHTML = '<h3>Tasting Notes</h3><p>No tasting notes available.</p>';
        }
        
        const foodPairingTab = document.getElementById('food-pairing-tab');
        if (product.foodPairing) {
            foodPairingTab.innerHTML = `<h3>Food Pairing</h3><p class="food-pairing-content">${product.foodPairing.replace(/\n/g, '<br>')}</p>`;
        } else {
            foodPairingTab.innerHTML = '<h3>Food Pairing</h3><p>No recommendations available.</p>';
        }
    }
    ...
    function setupProductSlideshow(mainContainer, thumbnailsContainer) {
        const slides = Array.from(mainContainer.querySelectorAll('.slideshow-item'));
        const thumbnails = Array.from(thumbnailsContainer.querySelectorAll('.thumbnail-item'));
        if (slides.length <= 1) {
            if (slides.length === 1) { slides[0].classList.add('active'); if (thumbnails.length === 1) thumbnails[0].classList.add('active'); }
            return;
        }
        if (mainContainer.querySelector('.slideshow-overlay')) return;
        const overlay = document.createElement('div');
        overlay.className = 'slideshow-overlay';
        ...
        function showSlide(index) {
            ...
            if (thumbnails[currentIndex]) thumbnails[currentIndex].classList.remove('active');
            currentIndex = (index + slides.length) % slides.length;
    ...
    function renderReviews(reviews) {
        ...
        container.innerHTML = '';
        if (reviews.length === 0) {
            container.innerHTML = '<p>No reviews yet for this product. Be the first to leave one!</p>';
            return;
        }
        reviews.forEach(review => {
            const reviewDate = review.createdAt?.toDate().toLocaleDateString() || 'N/A';
            container.innerHTML += `<div class="review-card"><div class="review-header"><div class="review-author">${review.userName || 'Anonymous'}</div><div class="stars">${generateStars(review.rating)}</div></div><h4 class="review-title">${review.title}</h4><p class="review-text">${review.text}</p><div class="review-date">${reviewDate}</div></div>`;
        });
    }

    async function renderReviewForm(user, product) {
        const container = document.getElementById('review-form-container');
        if (user) {
            container.innerHTML = `<h4>Leave a Review</h4><form id="review-form"><div class="form-group"><label>Your Rating</label><div class="star-rating"><input type="radio" id="star5" name="rating" value="5" required><label for="star5" title="5 stars">&#9733;</label><input type="radio" id="star4" name="rating" value="4"><label for="star4" title="4 stars">&#9733;</label><input type="radio" id="star3" name="rating" value="3"><label for="star3" title="3 stars">&#9733;</label><input type="radio" id="star2" name="rating" value="2"><label for="star2" title="2 stars">&#9733;</label><input type="radio" id="star1" name="rating" value="1"><label for="star1" title="1 star">&#9733;</label></div></div><div class="form-group"><label for="review-title">Review Title</label><input type="text" id="review-title" required></div><div class="form-group"><label for="review-text">Your Review</label><textarea id="review-text" required></textarea></div><button type="submit" class="btn-submit-review">Submit Review</button></form>`;
            document.getElementById('review-form').addEventListener('submit', (e) => handleReviewSubmit(e, user, product));
        } else {
            container.innerHTML = `<div class="login-prompt"><p>You must be <a href="#" id="login-link-in-review">signed in</a> to leave a review.</p></div>`;
            document.getElementById('login-link-in-review').addEventListener('click', (e) => { e.preventDefault(); document.getElementById('login-modal').style.display = 'block'; });
        }
    }
    ...
    async function handleReviewSubmit(e, user, product) {
        ...
        const reviewData = { userId: user.uid, userName: user.displayName || user.email.split('@')[0], rating: Number(rating), title: form['review-title'].value, text: form['review-text'].value, createdAt: serverTimestamp() };
    ...
    if (!productSlug) { document.querySelector('.product-section .container').innerHTML = '<h2>Product not found.</h2>'; return; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', function(e) {
        ...
        if (cartButton && !cartButton.id.includes('add-to-cart-btn')) { // Avoid conflict with product page main button
    ...
    // --- Page-Specific Initializer "Router" ---
