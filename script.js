...
// --- ИСПРАВЛЕНО ---
// Теперь функция правильно получает slug из URL вида /catalog/category/product-slug
function getProductSlugFromUrl() {
    const path = window.location.pathname;
    // Разделяем путь на части по символу "/" и отфильтровываем пустые значения
    const parts = path.split('/').filter(part => part);
    // Slug - это всегда последняя часть URL
...
// --- ИСПРАВЛЕНО ---
// Теперь функция генерирует "чистый" URL вида /catalog/category-slug/product-slug
function createProductCardHTML(prod) {
    if (!prod || !prod.slug || !prod.category) return '';
    const categorySlug = generateSlug(prod.category);
    const productUrl = `/catalog/${categorySlug}/${prod.slug}`; // ИЗМЕНЕННАЯ СТРОКА
...
function initIndexPage() {
    ...
    function renderCategoryCarousels(products) {
        ...
        CATEGORIES_TO_DISPLAY.forEach(categoryName => {
            ...
            animationObserver.observe(carouselWrapper);
            setupCarouselNavigation(carouselWrapper);
            carouselWrapper.querySelectorAll('.slideshow-container').forEach(setupSlideshow);
            carouselWrapper.querySelectorAll('.product-card').forEach(card => animationObserver.observe(card));
        });
    }
    ...
    function renderFeaturedProducts(products) {
        ...
        featuredProductsGrid.querySelectorAll('.product-card').forEach(card => {
            ...
            setupSlideshow(card.querySelector('.slideshow-container'));
        });
    }
...
function initCatalogPage() {
    ...
    function renderProducts(reset = false) {
        ...
        productsGrid.querySelectorAll('.product-card:not(.is-visible)').forEach(card => {
            animationObserver.observe(card);
            setupSlideshow(card.querySelector('.slideshow-container'));
        });
