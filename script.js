...
import { getFirestore, collection, getDocs, doc, getDoc, setDoc, query, where, updateDoc, runTransaction, arrayUnion, arrayRemove, writeBatch, limit } from "https://www.gstatic.com/firebasejs/9.15.0/firebase-firestore.js";
...
// ИСПРАВЛЕНО: Observer теперь может повторно активировать анимацию
const animationObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            entry.target.classList.add('is-visible');
        } else {
            // Опционально: убирать класс, чтобы анимация повторялась при прокрутке вверх
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Page Router ---
    // Проверяем наличие уникального элемента для главной страницы
    if (document.getElementById('hero-section')) {
        initIndexPage();
    }
    // Здесь будут проверки для других страниц (каталог, товар)
    // else if (document.getElementById('filters-container')) { initCatalogPage(); }
    // else if (document.querySelector('.product-container')) { initProductDetailPage(); }

    // --- UX & ANIMATION ENHANCEMENTS ---

    // ИСПРАВЛЕНО: Восстановлен MutationObserver для динамического контента
    ...
    // ИСПРАВЛЕНО: Восстановлена функция для прогресс-баров каруселей
    function enhanceCarousels() {
        document.querySelectorAll('.category-carousel-wrapper').forEach(wrapper => {
            ...
            if (!carousel || wrapper.querySelector('.carousel-progress')) return; // Не добавлять, если уже есть
    ...
    // Наблюдаем за контейнером каруселей, чтобы улучшить их после добавления
        ...
        const observer = new MutationObserver((mutationsList, obs) => {
            for(const mutation of mutationsList) {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    enhanceCarousels();
                    obs.disconnect(); // Запускаем один раз после добавления каруселей
                    break;
                }
            }
        });
        observer.observe(carouselsContainer, { childList: true });
    }

    // ИСПРАВЛЕНО: Восстановлено плавное закрытие мобильного меню
    ...
    // Применяем анимацию ко всем статическим элементам на странице
    document.querySelectorAll('.animate-on-scroll').forEach(el => animationObserver.observe(el));
    // Применяем MutationObserver к динамическим контейнерам
