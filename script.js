...
async function initCatalogPage() {
    ...
    function updateDependentFilters() { /* ... logic to disable/enable filters based on selection ... */ }
    ...
    function addEventListeners() {
        ...
        filtersContainer.addEventListener('input', e => {
            if (e.target.id === 'price-slider-min' || e.target.id === 'price-slider-max') {
                ...
                // ... slider range update logic ...
            }
        });

        sortSelect.addEventListener('change', runFilterAndSort);
        loadMoreBtn.addEventListener('click', () => renderProducts(false));
        resetFiltersBtn.addEventListener('click', () => { /* ... reset logic ... */ runFilterAndSort(); });
        activeFiltersContainer.addEventListener('click', e => { /* ... remove pill logic ... */ });
        document.querySelector('.mobile-filters-btn')?.addEventListener('click', () => { /* ... toggle sidebar ... */ });
    }
...
async function initProductPage() {
    ...
    async function fetchRelatedProducts(currentProduct) {
        ...
        const querySnapshot = await getDocs(q);
        return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })).filter(p => p.id !== currentProduct.id).slice(0, 4);
    }
    
    function renderProduct(product) {
        // ... (вся логика рендеринга страницы товара из вашего product.html)
    }

    function renderRelatedProducts(products) {
        // ... (вся логика рендеринга похожих товаров)
    }
    ...
    // ... (добавление всех event listeners для страницы товара)
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (pageInitFunction) {
        ...
        // Для страниц без специальной логики (например, "About Us")
        updateHeaderCounters();
        loader.classList.add('hidden');
        mainContent.classList.add('loaded');
    }
