...
function createProductCardHTML(prod) {
    ...
    const productUrl = `/catalog/${categorySlug}/${prod.slug}.html`; // Добавил .html для совместимости
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            mutations.forEach(mutation => {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) {
                        ...
                        node.querySelectorAll('.slideshow-container').forEach(setupSlideshow);
                    }
                });
            });
        });
    ...
    function enhanceCarousels() {
        document.querySelectorAll('.category-carousel-wrapper').forEach(wrapper => {
            ...
            if (!carousel || wrapper.querySelector('.carousel-progress')) return;
    ...
    // --- PAGE ROUTER & FINAL INITIALIZATION ---
