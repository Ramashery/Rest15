...
// ИЗМЕНЕНО: Функция теперь получает slug из пути URL, а не из GET-параметра.
function getProductSlugFromUrl() {
    const path = window.location.pathname;
    // Разделяем путь на части по символу "/" и отфильтровываем пустые значения
    const parts = path.split('/').filter(part => part);
    // Slug - это всегда последняя часть URL на странице товара
...
function createProductCardHTML(prod) {
    ...
    // ИЗМЕНЕНО: Формируем "чистый" URL вместо URL с GET-параметром.
