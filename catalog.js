...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function runFilterAndSort() {
        ...
        // Обновляем заголовок вкладки браузера после применения фильтров для UX
        let title = "Wine Catalog";
        const filters = state.filters;
        const capitalize = (s) => s && s.charAt(0).toUpperCase() + s.slice(1);
        const currentParams = new URLSearchParams(window.location.search);

        if (filters.category && filters.category.length > 0) {
            title = filters.category.map(capitalize).join(', ') + " Wines";
        } else if (currentParams.has('search')) {
            title = `Search results for "${currentParams.get('search')}"`;
        } else if (filters.region && filters.region.length > 0) {
            title = filters.region.map(capitalize).join(', ') + " Wines";
            if (filters.country && filters.country.length > 0) {
                title += " from " + filters.country.map(capitalize).join(', ');
            }
        } else if (filters.country && filters.country.length > 0) {
            title = "Wines from " + filters.country.map(capitalize).join(', ');
        }
        document.title = title + " | VinoElite";
    }
    ...
    // =================================================================================
    // === ИСПРАВЛЕНИЕ ЗДЕСЬ: Добавлена проверка на наличие цен ===
    // =================================================================================
    function renderPriceFilter() {
        // 1. Отбираем только те цены, которые являются корректными числами
        const prices = allProducts
            .map(p => p.price)
            .filter(price => typeof price === 'number' && isFinite(price));

        // 2. Если нет ни одной корректной цены, не рендерим фильтр вообще, чтобы избежать ошибки
        if (prices.length === 0) {
            // Устанавливаем безопасные значения по умолчанию
            state.filters.price.min = 0;
            state.filters.price.max = 9999;
            return; 
        }

        // 3. Теперь этот код безопасен, так как он работает только с корректными числами
    ...
    function addEventListeners() {
        ...
        // --- LOGIN MODAL & GUEST INTERACTIONS ---
        ...
        // --- ДОБАВЛЕНО: DESKTOP SEARCH MODAL LOGIC (IMPROVED FOR CATALOG) ---
