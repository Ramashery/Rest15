...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // НОВОЕ: Единый объект состояния для управления запросами
    const state = {
        filters: {
        ...
        sorting: 'popularity',
        lastVisible: null, // Для пагинации
        isLoading: false, // Для предотвращения двойных запросов
        totalCount: 0,
        displayedCount: 0
    };
    ...
    // НОВОЕ: Главная функция для получения и отрисовки товаров
        ...
        try {
            ...
            Object.keys(state.filters).forEach(key => {
                ...
                if (key === 'price') {
                    ...
                    // Поиск по-простому. Для сложного поиска нужен Algolia/Typesense
                    // Этот запрос требует индекса для каждого поля
                    // queries.push(where("keywords", "array-contains", value.toLowerCase()));
            ...
            switch (state.sorting) {
                ...
                default: finalQuery = query(finalQuery, orderBy("popularity", "desc")); break; // Сортировка по умолчанию
            }
            ...
            // 7. Обновляем состояние для следующей страницы
            ...
            // Для получения общего количества нужен отдельный запрос, что дорого.
            // Пока будем показывать только текущее состояние.
            ...
            // ВАЖНО: Firestore часто в ошибке дает ссылку для создания индекса.
    ...
    // ИЗМЕНЕНО: Функция теперь просто добавляет товары в DOM
    function renderProducts(products) {
        products.forEach(prod => {
            const card = document.createElement('div');
            card.className = 'product-card animate-on-scroll';
            // ... (остальной код для создания карточки товара остался без изменений)
            let subtitle = '';
        ...
        updateHeaderCounters(); // Обновляем счетчики в шапке
    }

    // НОВОЕ: Функция для обновления счетчиков и кнопки "Загрузить еще"
    function updateCountersAndButtons(isLastPage) {
        // В идеале нужно знать общее число, но это требует доп. запроса.
        // Поэтому пока просто показываем сколько загружено.
    ...
    // НОВОЕ: Функция для инициализации фильтров (загрузка уникальных значений)
    async function initializeFilters() {
        // Это упрощенный вариант. В идеале, эти данные должны храниться в отдельном документе "metadata"
        // чтобы не делать полный скан коллекции. Но для начала это сработает.
        const filterValues = {
    ...
    // НОВОЕ: Функция для отрисовки UI фильтров
    function renderFilterUI(filterValues) {
        ...
        filtersContainer.innerHTML = ''; // Очищаем
        // renderPriceFilter(); // Добавьте вашу функцию рендера фильтра по цене
    ...
    function handleFilterChange() {
        ...
        // Заполняем state на основе активных чекбоксов
        ...
        fetchAndRenderProducts(false); // Запускаем новый поиск
    }

    // Главная функция инициализации
    async function main() {
        addEventListeners();
        await initializeFilters(); // Сначала загружаем и рендерим фильтры
        await fetchAndRenderProducts(); // Затем загружаем первую страницу товаров
        loader.classList.add('hidden');
        mainContent.classList.add('loaded');
    }

    function addEventListeners() {
        // ... (весь ваш код для мобильного меню, модальных окон и т.д. остается здесь без изменений)
        
        // ИЗМЕНЕНО: Логика обработчиков
        ...
        // Остальные обработчики (клик по карточке, в корзину и т.д.)
