...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function buildFirestoreQuery() {
        ...
        // Применяем фильтры по категориям, странам и т.д.
        Object.keys(state.filters).forEach(key => {
            const values = state.filters[key];
            if (key !== 'price' && Array.isArray(values) && values.length > 0) {
                q = query(q, where(key, 'in', values));
            }
        });
        
        // Применяем фильтр по цене, только если он был изменен пользователем.
        // Для этого мы проверяем, отличаются ли текущие значения от начальных.
        const minSlider = document.getElementById('price-slider-min');
        const maxSlider = document.getElementById('price-slider-max');
        
        if (minSlider && maxSlider) {
            const initialMin = parseInt(minSlider.min);
            const initialMax = parseInt(maxSlider.max);

            // Если пользователь сдвинул хотя бы один ползунок, применяем фильтр по цене.
            if (state.filters.price.min > initialMin || state.filters.price.max < initialMax) {
    ...
    function applySortingAndPaginationToQuery(q) {
        switch (state.sorting) {
            ...
            default: q = query(q, orderBy('name')); 
        }
    ...
    function renderSweetnessFilter() { /* ... (без изменений) ... */ }
    ...
    function renderFilters() {
        ...
        filterDefinitions.forEach(({key, title}) => {
            if (key === 'sweetness') { /* ... */ } 
                ...
                const options = getUniqueValues(key, allProductsMetadata);
    ...
    function renderPriceFilter() {
        const prices = allProductsMetadata.map(p => p.price);
    ...
    function updateDependentFilters() { /* ... (без изменений) ... */ }
    function updateOptionsVisibility(filterKey, availableOptions) { /* ... (без изменений) ... */ }
    ...
    function generateSlug(text) { return text ? text.toString().toLowerCase().replace(/\s+/g, '-').replace(/[^\w\-]+/g, '') : ''; }
    function parseUrlParams() { /* ... (без изменений) ... */ }
    function updateURL() { /* ... (без изменений) ... */ }
    function updateCheckboxesFromState() { /* ... (без изменений) ... */ }

    function addEventListeners() {
        /* ... (вся логика мобильного меню, модальных окон, поиска - без изменений) ... */

        productsGrid.addEventListener('click', function(e) { /* ... (без изменений) ... */ });

        filtersContainer.addEventListener('change', e => {
            if (e.target.type !== 'checkbox' && !e.target.id.startsWith('price-slider')) return;
        ...
        activeFiltersContainer.addEventListener('click', e => { /* ... (без изменений) ... */ });
        ...
        resetFiltersBtn.addEventListener('click', () => { /* ... (без изменений) ... */ runFilterAndSort(); });

        /* ... (остальные обработчики без изменений) ... */
        loadMoreBtn.addEventListener('click', () => fetchAndRenderProducts(false));
